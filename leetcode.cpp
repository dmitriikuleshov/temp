...
class SuffixTree {
  ...
  public:
    ...
    // node starts at string2), a vector of suffix node ids, length, and any
    // leaf node id
    std::tuple<bool, bool, std::vector<size_t>, size_t, size_t>
    findAllLCSInNode(SuffixTreeNode *node, size_t dividerIndex, size_t currentDepth) {
        ...
        for (const auto &[_, edge] : node->next) { // iterate over all edges
            size_t nodeId = edge->node->id;
            size_t edgeLength = edge->getLength();
            leafId = nodeId;

            if (edge->node->empty()) {       // leaf node at this edge => check its id
                                             // and decide whether to keep it or not
                if (nodeId < dividerIndex) { // suffix is in `string1`
                    inString1 = true;
                } else if (nodeId > dividerIndex &&
                           nodeId != this->inputStringLength - 1) { // suffix is in `string2`
                    inString2 = true;
                }
            } else { // edge ends in an internal node => go deeper, save edge's
                     // string position, indecies and length
                auto [currentInString1, currentInString2, currentIndecies, currentLength,
                      currentLeafId] =
                    findAllLCSInNode(edge->node, dividerIndex, currentDepth + edgeLength);
                ...
                if (currentInString1 && currentInString2 && currentLength >= maxLength) {
                    if (currentLength > maxLength) { // new length is bigger => save only this
                                                     // edge's start
                        maxLength = currentLength;
                        indecies.clear();
                    }
                    indecies.reserve(indecies.size() + currentIndecies.size());
                    for (size_t index : currentIndecies) {
  ...
  public:
    ...
    std::pair<std::vector<size_t>, size_t> findAllLCS(const std::string &pattern) {
        ...
        for (size_t suffixIndex = 0; suffixIndex < patternLength;
            ...
            for (index = suffixIndex; index < patternLength; ++index) {
                ...
                if (this->activeLength == 0) {
                    const auto entry = this->activeNode->next.find(currentChar);
                    if (entry != this->activeNode->next.end()) { // there is an edge starting with
                                                                 // this char
            ...
            if (maxSubstringLength >=
                patternLength - suffixIndex) { // there are no strings, with length bigger
    ...
    // find all common substrings in `string1` and `string2` with maximal length
    // in the suffix tree return indecies of starts of substrings in the input
    ...
    std::pair<std::vector<size_t>, size_t> findAllLCS() {
        ...
        // suffix is in the text, if id > index('#') => the suffix is in the
        // pattern if the suffix is in the pattern and is in text it is a common
        // substring
    ...
    size_t findWordAndAccumulateDepth(const std::string &wordA) {
        size_t accumulatedDepth = 0;

        // Start from the root node
        SuffixTreeNode *currentNode = this->root;
        size_t currentDepth = 0;
        size_t wordIndex = 0;
        size_t wordLength = wordA.length();

        // Always count the root if it's internal (which it always is for non-empty trees)
        // if (!currentNode->empty()) {
        //     // Multiply depth by number of edges before adding
        //     accumulatedDepth += currentDepth * currentNode->next.size();
        // }

        while (wordIndex < wordLength) {
            // Get the current character from word A
            char currentChar = wordA[wordIndex];

            // Check if current node has an edge starting with current character
            auto edgeIt = currentNode->next.find(currentChar);
            if (edgeIt == currentNode->next.end()) {
                // Character not found - word A is not in the tree
                return 0;
            }
            ...
            size_t compareLength = std::min(edgeLength, wordLength - wordIndex);

            // Compare characters along the edge
            for (size_t i = 0; i < compareLength; i++) {
                size_t stringIndex = currentEdge->start + i;
                if (stringIndex >= this->inputStringLength ||
                    this->inputString[stringIndex] != wordA[wordIndex + i]) {
                    return 0; // Character mismatch
                }
            }

            // Update position in word A
            wordIndex += compareLength;
            ...
            // count
            if (!currentNode->empty() && currentNode != root) {
                // Multiply current depth by number of edges before adding to accumulated depth
                accumulatedDepth +=
                    currentDepth * dfsLeavesCount(currentNode, currentDepth, wordIndex);
            }

            // If we haven't traversed the entire edge, we're done
            if (compareLength < edgeLength) {
                return (wordIndex == wordLength) ? accumulatedDepth : 0;
            }
        }
...
class Solution {
  public:
    long long sumScores(std::string s) {
        ...
        // Since we're building by prepending, s_i is the suffix of length i starting at
        // position n-i The score is LCP(suffix starting at n-i, suffix starting at 0)

        // Build suffix tree
        SuffixTree tree(s);

        long long totalScore = tree.findWordAndAccumulateDepth(s) + s.length();
        // For each i from 1 to n, compute LCP between suffix starting at n-i and suffix
        // starting at
        // 0
...
int main() {
    ...
    std::cout << "Output: " << sol.sumScores(s1) << std::endl;
    std::cout << "Expected: 9" << std::endl << std::endl;

    // Test case 2
    std::string s2 = "cbccbbbbcccbbcabcaccbbbaccbbccabbaabbbaacaacbacaccaabccbacbbbccacc";
    std::cout << "Input: " << s2 << std::endl;
    std::cout << "Output: " << sol.sumScores(s2) << std::endl;
    std::cout << "Expected: 98" << std::endl;
