...
class AbstractNode {
  public:
    explicit AbstractNode(int id)
        : Id(id), Port(ZmqUtils::PORT_TO_BIND_FROM), outerNodes(), context(1),
          Receiver(context, zmq::socket_type::rep) {
        occupyPort();
    }
    ...
    pid_t addChild(int id, int registerPort) {
        ...
        if (child == 0) {
            execl("server", to_string(id).c_str(), to_string(Id).c_str(),
                  to_string(Port).c_str(), to_string(registerPort).c_str(),
                  NULL);
        }
        return child;
    }
  ...
  protected:
    ...
    void occupyPort() { Port = ZmqUtils::occupyPort(Receiver); }
};
