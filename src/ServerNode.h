...
class ServerNode : public AbstractNode {
  public:
    ServerNode(int id, int _parentId, int _parentPort, int _registerPort)
        : AbstractNode(id), parentId(_parentId), parentPort(_parentPort),
          parentRegPort(_registerPort),
    ...
    void run() {
        ...
        // std::cout << "port " << Port << " parent port " << parentPort <<
        // std::endl;
        ...
        while (true) {
            ...
            if (!messageIn.receiveMessage(Receiver,
            ...
            case MessageTypes::HEARTBIT_REQUEST:
                // std::cout << "Heartbit request arrived at server " << Id <<
                // std::endl;
  ...
  private:
    ...
    void selfRelation(int _registerPort) {
        ...
        Message message(MessageTypes::RELATE_REQUEST, Id, parentId,
                        2 * sizeof(int) + sizeof(pid_t), relationBody);

        message.sendMessage(sender);

        Message response;
        if (response.receiveMessage(sender, std::chrono::milliseconds(1000))) {
            std::cout << "Node " << Id << " successfully registered to parent "
                      << parentId << std::endl;
        } else {
            std::cerr << "Node " << Id << " failed to register to parent "
                      << parentId << std::endl;
        }
    ...
    Message createProcessor(Message &message) {
        ...
        if (outerNodes.empty()) {
            std::cerr << "No child nodes available to handle request."
                      << std::endl;
        ...
        for (auto &server : outerNodes) {
            ...
            if (!result.receiveMessage(sender,
                                       std::chrono::milliseconds(2000))) {
                std::cerr << "Timeout waiting for response from node "
                          << server.first << std::endl;
                sender.disconnect(
                    ZmqUtils::getOutputAddress(server.second.ReceiverPort));
                continue;
    ...
    Message execProcessor(Message &message) {
        ...
        // std::cout << "EXEC INFO: Id " << Id << "target id " << data.id <<
        // std::endl;
        if (data.id == Id) {
        ...
        for (auto server : this->outerNodes) {
            ...
            if (!result.receiveMessage(requester,
                                       std::chrono::milliseconds(2000))) {
    ...
    Message quitProcessor(Message &message) {
        ...
        for (auto server : outerNodes) {
            ...
            if (!messageResult.receiveMessage(
                    request, std::chrono::milliseconds(2000))) {
                request.disconnect(
                    ZmqUtils::getOutputAddress(server.second.ReceiverPort));
                continue;
            }
