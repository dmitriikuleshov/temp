...
class Message {
  public:
    Message(MessageTypes messageType, int senderId, int receiverId,
            int _sizeOfBody, void *_body)
        : messageType(messageType), senderId(senderId), recieverId(receiverId),
          sizeOfBody(_sizeOfBody), body(nullptr) {
        if (sizeOfBody > 0) {
    ...
    void sendMessage(zmq::socket_t &socket) {
        ...
        // send compound message using SNDMORE flag in every message part except
        // the last one
    ...
    void receiveMessage(zmq::socket_t &socket) {
        ...
        for (bool t = false; !t; t = getMessage(socket, receivedType,
                                                receivedHeader, receivedBody)) {
    ...
    bool receiveMessage(zmq::socket_t &socket, std::chrono::milliseconds time) {
        ...
        for (; !t && passed < time.count();
             t = getMessage(socket, receivedType, receivedHeader,
                            receivedBody)) {
